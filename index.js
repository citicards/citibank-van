...
function userEnteredCorrectLoginNameAndPassword(loginResults) {
  if(loginResults && loginResults.data.includes('SessionId')) {
    return true;
  }
  return false;
};
...
class VAN {
  ...
  login(username, password) {
    ...
    return this.getCookies()
      ...
      .then(loginResults => {
        if (userEnteredCorrectLoginNameAndPassword(loginResults)) {
          this.sessionId = new URLSearchParams(loginResults.data).get('SessionId');
          return this.getCards();
        }
        const err = new Error('login failed');
        err.message = loginResults.data;
        throw err;
      });
  }
  getCards() {
    return this.api.post('/Athena/WebServlet', `Password=${this.password}&Request=GetActiveCards&SessionId=${this.sessionId}&User=${this.username}&IssuerId=1&WebcardType=SLIM&Locale=en`)
      .then(getCardsResults => {
        const results = new URLSearchParams(getCardsResults.data);
        this.sessionId = results.get('SessionId');
        this.usersNumberOfCreditCards = Number(results.get('Total'));
        for (let i = 1; i <= this.usersNumberOfCreditCards; i++) {
          this.creditCards.push({
            cardholdersName: results.get(`CardholderName${i}`),
            lastFourOfCardNumber: results.get(`PAN${i}`),
            cardNickName: results.get(`Nickname${i}`),
            defaultCard: results.get(`DefaultCard${i}`) === 'Y',
            VCardId: results.get(`VCardId${i}`),
            CardType: results.get(`CardType${i}`)
          });
        }
